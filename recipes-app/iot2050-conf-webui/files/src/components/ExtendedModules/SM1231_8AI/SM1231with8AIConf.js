...
export default function SM1231with8AIConf ({ slotNum, configData, updateConfig }) {
  ...
  const unCheckAlarm = (event) => {
    const chIndex = parseInt(event.target.name.slice(-1), 10);
    if (configData.channels[chIndex].range.value === uiString.RANGE_3) {
      configData.channels[chIndex].underflow_alarm.value = false;
    } else if (configData.channels[chIndex].range.value === uiString.RANGE_2) {
      configData.channels[chIndex].open_wire_alarm.value = false;
    }
  };
